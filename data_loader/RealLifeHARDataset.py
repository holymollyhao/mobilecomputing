...
class RealLifeHARDataset(torch.utils.data.Dataset):
    # load static files
    ...
    def preprocessing(self):
        ...
        for idx in range(max(len(self.df) // WIN_LEN, 0)):
            ...
            # domain = self.df.iloc[idx * WIN_LEN, 54]
            # class_label = self.df.iloc[idx * WIN_LEN, 55]
            # domain = self.df.iloc[idx * WIN_LEN, 72]
            # class_label = self.df.iloc[idx * WIN_LEN, 73]
            # domain = self.df.iloc[idx * WIN_LEN, 90]
            # class_label = self.df.iloc[idx * WIN_LEN, 91]
    ...
    def class_to_number(self, label):
        dic = {'Inactive': 0, 'Active': 1, 'Walking': 2, 'Driving': 3}
        return dic[label]
