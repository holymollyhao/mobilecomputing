...
conf.args.gpu_idx = 0
conf.args.opt={}
conf.args.opt['num_class']=14
device = torch.device("cuda:{:d}".format(conf.args.gpu_idx) if torch.cuda.is_available() else "cpu")
'''



feature_flatten_dim = 512 #128 len
input_channel_dim = 1
...
class Extractor(nn.Module):
    ...
    def __init__(self):
        ...
        self.feature = nn.Sequential(
            ...
            nn.Conv1d(256, 512, kernel_size=6, stride=2),
            nn.MaxPool1d(2),
            nn.ReLU(True),
            nn.BatchNorm1d(512),
...
class Class_Classifier(nn.Module):
    ...
    def __init__(self):
        ...
        self.class_classifier = nn.Sequential(
            nn.Linear(feature_flatten_dim, conf.ICSROpt['num_class']))
...
class AdversarialNetwork(nn.Module):
    def __init__(self, lr_mult=10, decay_mult=2):
        ...
        self.ad_layer1 = nn.Linear(feature_flatten_dim * conf.ICSROpt["num_class"], hidden_size)
...
if __name__ == '__main__':
    ...
    out = fe(torch.zeros((10, 1, 32000)))

    print(out.shape)

    out = cc(fe(torch.zeros((10,1,32000))))
    print(out.shape)
