...
for i in 1; do
  ...
  python main.py --gpu_idx ${i} --dataset kitti_sot --method Src --src ${SRC} --tgt ${TGT} --log_suffix ${LOG_SUFFIX}  --model KITTI_SOT_model --epoch 100 --update_every_x 64 --seed 0 --load_checkpoint_path ${CP_PATH} 2>&1 | tee raw_logs/${LOG_SUFFIX}_job${i}.txt &
done
wait
