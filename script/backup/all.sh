...
#DATE="220503-9_iabn_trainable_scale10"
#DATE="220503-10_iabn_trainable_scale100"
DATE="220503-11_iabn_trainable_scale1000"


#cifar10 cifar100 harth reallifehar extrasensory kitti_sot
#Src FT_all CDAN SHOT FeatMatch TT_BATCH_STATS PseudoLabel TENT T3A COTTA
#harth reallifehar extrasensory
if [ "${SERVER}" = "suzy" ]; then
  DATASETS="cifar10 cifar100 harth reallifehar extrasensory kitti_sot"
  METHODS="Ours"
elif [ "${SERVER}" = "iu" ]; then
  DATASETS="cifar10 cifar100 harth reallifehar extrasensory kitti_sot"
  METHODS="Ours"
...
#      LOG_SUFFIX_SRC="log/kitti_sot/Src/src_2d_detection/tgt_2d_detection/220419-1_kitti_sot_src-2d_detection_tgt-2d_detection_lr1e-4_pretrained_ep100_uex64_s0/cp/cp_last.pth.tar"
      ...
      CP1="log/kitti_sot/Src/tgt_rain-200/220503-3_kitti_iabn_fine-tune_ep100_uex64_k3_s0/cp/cp_last.pth.tar"
      CP2="log/kitti_sot/Src/tgt_rain-200/220503-3_kitti_iabn_fine-tune_ep100_uex64_k4_s0/cp/cp_last.pth.tar"
      CP3="log/kitti_sot/Src/tgt_rain-200/220503-3_kitti_iabn_fine-tune_ep100_uex64_k5_s0/cp/cp_last.pth.tar"
      CP4="log/kitti_sot/Src/tgt_rain-200/220503-3_kitti_iabn_fine-tune_ep100_uex64_k10_s0/cp/cp_last.pth.tar"
      lr="0.0001"
...
#      LOG_SUFFIX_SRC="log/cifar10/Src/src_original/tgt_original/220421-1_cifar10_src-original_tgt-original_lr1e-4_pretrained_ep100_uex64_s0/cp/cp_last.pth.tar"
      ...
      if [ "${METHODS}" == "T3A_woBN" ]; then
        #TODO update source location for fused
        LOG_SUFFIX_SRC="${DATE}_${DATASET}_Src_fuse_lr_0.0001"
      else
        #TODO update source location for without fusing
        LOG_SUFFIX_SRC="${DATE}_${DATASET}_Src_nofuse_lr_0.0001"
      fi
      num_con=3
      MODEL="resnet50"
      lr="0.0001"
...
elif [ "${METHOD}" = "T3A" ] || [ "${METHOD}" = "T3A_woBN" ] ; then
  EPOCH="100"
  SEED="0" #"0 1 2"
  nsample=99999
  LOG_SUFFIX="${DATE}"

  if [ "${METHOD}" = "T3A_woBN" ] ; then # T3A without bn usage, assumes it is only called by vlcs, officehome, pacs
    LOG_SUFFIX="${DATE}_fuse"
    python script.py \
      --validation \
      --num_concurrent $num_con \
      --log_as_file \
...
--log_suffix ${LOG_SUFFIX}"
  ...
  elif [ "${DATASET}" = "vlcs" ] || [ "${DATASET}" = "officehome" ] || [ "${DATASET}" = "pacs" ] ; then # T3A with bn usage
    LOG_SUFFIX="${DATE}_nofuse"
    python script.py \
        --validation \
        --num_concurrent $num_con \
        --log_as_file \
