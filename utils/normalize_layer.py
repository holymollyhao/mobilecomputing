...
def get_normalize_layer(dataset):
    ...
    if dataset in ['kitti_sot','vlcs','pacs','officehome', 'tinyimagenet', 'visda', 'terra_incognita', 'domainnet']:
        return NormalizeLayer(_IMAGENET_MEAN, _IMAGENET_STDDEV)
    if dataset == "cifar10":
        return NormalizeLayer(_CIFAR10_MEAN, _CIFAR10_STDDEV)
    elif dataset == "cifar100":
        return NormalizeLayer(_CIFAR100_MEAN, _CIFAR100_STDDEV)
    elif dataset in ['svhn', 'cmnist', 'rmnist', 'mnist']:
        return NormalizeLayer(_Default_MEAN, _Default_STDEV)
    else:
        return None